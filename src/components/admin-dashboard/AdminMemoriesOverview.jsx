...
export default function AdminMemoriesOverview() {
    const { id } = useParams()
    ...
    const handleTogglePublic = async (checked) => {
        ...
        try {
            // Send the update to the backend, converting boolean to integer
            const response = await axios.patch(`${server}/tribute/update-visibility/${id}`, { 
                is_public: checked ? 1 : 0 
            });

            if (response.data.status === "success") {
                toast.success(`Tribute visibility set to ${checked ? 'public' : 'private'}`);
            } else {
   ...
   const handleSubmit = async () => {
        ...
        // Append tribute fields, excluding is_public, no_donations and handling nulls
        Object.entries(tribute).forEach(([key, value]) => {
            if (key !== 'is_public' && key !== 'no_donations' && key !== 'allow_anonymous' && key !== 'no_limit' && value !== null && value !== undefined) {
        ...
        try {
            const response = await axios.post(`${server}/admin/tribute/overview/update/${id}`, formDataToSubmit, {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (response.data.status === "success") {
                ...
                navigate("/admin/dashboard/main"); // Navigate to dashboard or tributes list
            } else {
                        ...
                        <>
                                                ...
                                                <div className="bg-[#f8f4f0] p-4 sm:p-6 rounded-lg space-y-6">
                                                    <div>
                                                        ...
                                                        <InputField
                                                            ...
                                                            id="customMemorialWebsite" // Ensure this ID is unique if you have another similar field
                                                            ...
                                                            onChange={handleInputChange} // Or make it non-editable if purely generated
                                                            icon={<LinkIcon className="h-4 w-4 text-[#fcd34d]" />}
                                                            disabled // This is the auto-generated one
                                                        />
                            ...
                            <CardFooter className="flex flex-col sm:flex-row justify-between items-center border-t p-3 sm:p-6 gap-3 sm:gap-4">
                                ...
                                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                                    ...
                                    <Button
                                        onClick={() => navigate(`/admin/dashboard/tribute-life/${id}`)}
